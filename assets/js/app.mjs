...
const PUBLIC_DEFAULT_EXAMPLE_ID='ADVANCED_WEB_502';
...
async function ensurePublicStartupProject(){
  ...
  if(!project){await openDefault('Se abrió automáticamente el ejemplo avanzado web para que puedas probar la generación local sin instalaciones.');return;}
  ...
  if(syntheticLegacy){
    ...
    await openDefault('Se abrió la candidata final web con un ejemplo avanzado compatible. Las copias locales anteriores se conservan como respaldo.');
  }
}
...
function renderExampleLibrary(){
  ...
  return `<section class="card example-library-intro"><div class="toolbar"><div><p class="eyebrow">Biblioteca sintética</p><h2 style="margin:0">Ejemplos para la versión web</h2><p class="muted">Todos los datos son ficticios. Todos los ejemplos ordinarios de esta biblioteca se generan localmente en el navegador.</p></div><span class="spacer"></span><button class="btn secondary" data-go="home">Volver al inicio</button></div></section><div class="grid two example-library" style="margin-top:16px">${cards}</div>${technicalMode?'':'<section class="card" style="margin-top:16px"><p class="muted">La versión pública usa motor web local; los diagnósticos internos solo aparecen en mantenimiento explícito.</p></section>'}`;
}
...
async function handleSubmit(e){
  ...
  try{
    ...
    if(form.dataset.form==='domain4'){ensureDraft();project.domain=parseDomain4Form(fd,project.domain);project=normalizeProject(project);appendAudit(project,'EDUCATIONAL_DOMAIN_4_UPDATED','Dominio educativo 4.0 actualizado.');changed('Dominio educativo guardado. Las capacidades avanzadas se revisarán con el motor web y la validación independiente.');render();return;}
...
async function handleCpSatResponseFile(file){
  try{const value=validateCpSatResult(JSON.parse(await file.text()));applyGenerationResult(value);notice('Respuesta técnica importada y revalidada.','success');}
  catch(error){notice(`No se pudo importar la respuesta técnica: ${error.message}`,'error');}
}
