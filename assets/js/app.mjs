...
function renderExampleLibrary(){
  ...
  const cards=rows.map(item=>{
    ...
    const engine=item.expectedEngine==='CP_SAT_REQUIRED'?'Necesita generación avanzada':item.expectedEngine==='CP_SAT_RECOMMENDED'?'Diagnóstico avanzado recomendado':'Generación automática';
    const badge=item.status==='REFERENCE'?'Referencia':item.status==='EXPECTED_INFEASIBLE'?'Caso diagnóstico':item.status==='MAINTENANCE_ONLY'?'Solo mantenimiento':'Preparado para generar';
  ...
  return `<section class="card example-library-intro"><div class="toolbar"><div><p class="eyebrow">P4 · biblioteca sintética</p><h2 style="margin:0">Centros de ejemplo</h2><p class="muted">Todos los datos son ficticios. centro canónico se conserva como referencia canónica y ningún ejemplo puede modificar su gate de regresión.</p></div><span class="spacer"></span><button class="btn secondary" data-go="home">Volver al inicio</button></div><div class="notice info"><b>${CANONICAL_REGRESSION_GATE}</b>: 502 asignaciones, 15 grupos, 29 docentes y 14 espacios deben permanecer intactos.</div></section><div class="grid two example-library" style="margin-top:16px">${cards}</div>${technicalMode?'':'<section class="card" style="margin-top:16px"><p class="muted">El escenario de estrés se muestra únicamente en Sistema → Mantenimiento avanzado.</p></section>'}`;
}
...
function publicGenerationMessage(run){
  ...
  if(r.status==='UNAVAILABLE')return 'La optimización avanzada local no está disponible. El horario guardado no se ha modificado.';
...
function renderGenerationExecution(latestRun){
  ...
  const technicalDetails=technicalMode?`<details class="generation-technical"><summary>Detalles técnicos de la orquestación</summary><p><code>${esc(latestRun.request?.contractVersion||latestRun.contractVersion||'')}</code> · estrategia ${esc(productInfo.strategy||'—')} · ${esc(fmtDuration(r.durationMs||0))}</p>${productInfo.capabilities?`<p>CP-SAT local: <b>${productInfo.capabilities.available?'disponible':'no disponible'}</b>${productInfo.capabilities.engine?.version?` · ${esc(productInfo.capabilities.engine.version)}`:''}</p>`:''}${productInfo.attempts?.length?`<div class="table-wrap"><table><thead><tr><th>Intento</th><th>Estado</th><th>Duración</th><th>Propuesta</th></tr></thead><tbody>${productInfo.attempts.map((row,index)=>`<tr><td>${index+1} · ${esc(row.engine)}</td><td>${esc(generationStatusLabel(row.status))}</td><td>${esc(fmtDuration(row.durationMs))}</td><td>${row.hasUsableProposal?'Sí':'No'}</td></tr>`).join('')}</tbody></table></div>`:''}<div class="toolbar" style="margin-top:12px"><button class="btn small secondary" type="button" data-action="export-generation-run" data-id="${esc(latestRun.request?.requestId||'')}">Exportar registro JSON</button></div></details>`:'';
  return `<div class="notice ${noticeClass} generation-result"><div class="toolbar"><div><b>${technicalMode?`Última generación: ${esc(generationStatusLabel(r.status))}`:`Resultado de generación: ${esc(generationStatusLabel(r.status))}`}</b><br><small>${esc(technicalMode?(r.message||''):publicGenerationMessage(latestRun))}</small></div><span class="spacer"></span><b>${esc(fmtDuration(r.durationMs||0))}</b></div>${productInfo.diagnosticStatus==='INFEASIBLE'?'<p><b>Diagnóstico avanzado:</b> no existe una solución completa con las restricciones obligatorias modeladas.</p>':''}${metrics}${technicalDetails}</div>`;
}
